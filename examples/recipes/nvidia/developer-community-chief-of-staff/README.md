...
| `NEMOCLAW_INFERENCE_PREFLIGHT` | `1` | Validates the inference endpoint, credential, and model with one bounded, one-token completion before sandbox creation. Set to `0` only for intentional offline setup or an endpoint that cannot support verification. |
